function [phi] = semi_infinite_TR(mua, musp, n, rsd, rsd2, T, dt, boundary)     %added rsd2
...
elseif strcmp(boundary,'EBC-Robin')
    ...
    Phi(:,2) = cm/((4*pi*D*cm*t).^(3/2)).*exp(-mua*cm*t).*(exp(-(rsd^2)./(4*D*cm*t)) - exp(-((norm([srcpos(:,1) srcpos(:,2)-z0-2*zb]-detpos))^2)./(4*D*cm*t)));
