function [phi] = semi_infinite_TR_tddiffusion(mua, musp, n, source, detector, T, dt, boundary)     %added rsd2
...
elseif strcmp(boundary,'EBC-Robin')
    A = robin(n);
    zp = z0 + 4*A*D;
    rsd3 = getdistance([source(:,1) source(:,2)+z0], detector);
    rsd4 = getdistance([source(:,1) source(:,2)-zp], detector);
    %phi(:,2) = 0.5 * (4*pi*D*cm)^(-3/2) * t.^(-5/2) .* exp(-mua*cm*t) .* (z0 * exp(-(z0^2 + rsd^2)./(4*D*cm*t)) + zp * exp(-(zp^2 + rsd^2)./(4*D*cm*t)));
    %phi(:,2) = cm * (4*pi*D*cm*t).^(-3/2) .* exp(-mua*cm*t) .* (exp(-(z0^2 + rsd^2)./(4*D*cm*t)) - exp(-(zp^2 + rsd^2)./(4*D*cm*t))); %Letizia: elementwise power or not?
    phi(:,2) = cm./((4*pi*D*cm*t).^(3/2)).*exp(-mua*cm*t).*(exp(-(rsd3^2)./(4*D*cm*t)) - exp(-(rsd4^2)./(4*D*cm*t)));
